...
_TABLE_NOTES: dict[tuple[str, str], str] = {
    ...
    ("meta_organic", "v_ig_profile_monthly"): (
        ...
        "may be NULL where the source can't be trusted: before Aug 2025 (collection gap — "
        "reach IS populated back to Jan 2025), and where a `-1` sentinel appeared. "
        "NULL means 'not measured', NEVER zero — do not report it as zero, and exclude "
        "those months from trends and averages. "
        "Two rows are physically impossible and the view nulls the column actually at "
        "fault, not always the same one: hyfin.mke 2026-02 has corrupt ENGAGEMENTS "
        "(shares 70x its neighbours) so `engagements` is NULL and reach is kept; "
        "hyfin.mke 2026-07 has corrupt REACH (accounts_engaged 6,135 exceeded reach 2,213, "
        "and you cannot engage with a post you were never shown) so `reach` is NULL and "
        "engagements are kept. "
        "`is_complete` = FALSE for the month still in progress: Coupler re-pulls the "
        "current month nightly, so that row covers only the days elapsed so far. "
        "EXCLUDE `is_complete = false` rows from any trend, total, or month-over-month "
        "comparison — summing 9 days against 30 reads as a collapse. `period_end` gives "
        "the actual window. "
        "`accounts_engaged` only starts Dec 2025. `engagement_rate` = engagements/reach. "
